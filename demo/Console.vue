<template>
    <aside :class="{expanded, 'veui-console': true}">
        ...
        <section class="output" ref="logList">
            <pre class="log" v-for="(log, index) in logs" :key="index"><template v-if="log != null"><div
                    v-if="log instanceof String">{{log}}</div><div class="line" v-else v-for="line in log"
                                                                   v-html="format(line)"></div></template><template
                    v-else v-html="format(log)"></template></pre>
        </section>
    </aside>
</template>
...
<script>
    ...
    import 'veui-theme-bi/icons/angle-up'
    import 'veui-theme-bi/icons/angle-down'
